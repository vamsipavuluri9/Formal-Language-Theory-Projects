...
## File Structure
- **MainProgram.c**: The main program that handles the complete flow—reading a pattern, constructing the NDFSM, converting it to DFSM, and simulating the input string.
...
2. Prepare the `pattern1.txt` and `input1.txt` files.
   ...
   - **input1.txt**: The file should contain an input string you wish to check against the DFSM (e.g., `aabcbcaaaacccc`).
...
- `input1.txt`: Contains the input string `aabcbcaaaacccc`.
